...
pub struct DapAdapter<R> {
    ...
    output_thread: Option<tokio::task::JoinHandle<Result<(), std::io::Error>>>,
...
async fn output_thread<W: AsyncWrite>(
    write: W,
...
impl<R: AsyncRead> DapAdapter<R> {
    ...
    pub(crate) fn new<W: AsyncWrite + Send + 'static>(
        ...
        let output_thread = tokio::spawn(output_thread(writer, rx));
